...
norm.plot<-function(analysis_type = analysis_type){
  ...
  for(j in 1:length(analysis_type$analysis)){
    ...
    if(config_json$keep_not_compared_samples_for_normalization[1] == TRUE){
      ppnc.files <- plot.dt(analysis.type = analysis_type$analysis[j], comp.folder = "PCA",path.prefix = "../", path.suffix = "/report_data/", pattern1 = "pre_post_norm_counts.svg", pattern2 = "pre_post_norm_counts.svg")
    }else{
...
pca_heatmap.plot<-function(analysis_type = analysis_type, comparison = comparison){
  ...
  if (dir.exists(paste0("../",analysis_type$DE[1],"/PCA"))) {
    ...
    for(j in 1:length(analysis_type$name)){
      output<-c(output,cat("##### ",analysis_type$name[j],"\n"))
      allpca.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = "PCA",path.prefix = "../", path.suffix = "/report_data/", pattern1 = "sample_to_sample_PCA.svg", pattern2 = "sample_to_sample_PCA_batch.svg")
      allheatmap.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = "PCA",path.prefix = "../", path.suffix = "/report_data/", pattern1 = "heatmaps_samples_log2.png", pattern2 = "heatmaps_samples_log_batch.png")
