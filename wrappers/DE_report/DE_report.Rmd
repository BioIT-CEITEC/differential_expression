...
analysis_type <- analysis_type[name %like% "featureCount", name := gsub("_"," ",name)]
...
analysis_type <- analysis_type[name %like% "mirbase", name := gsub("_"," ",name)]
...
norm.plot<-function(analysis_type = analysis_type){
  ...
  for(j in 1:length(analysis_type$analysis)){
    ...
    if(config_json$keep_not_compared_samples_for_normalization[1] == TRUE){
      ppnc.files <- plot.dt(analysis.type = analysis_type$analysis[j], comp.folder = "PCA",path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "pre_post_norm_counts.svg", pattern2 = "pre_post_norm_counts.svg")
    }else{
      ppnc.files <- plot.dt(analysis.type = analysis_type$analysis[j], comp.folder = comparison, path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "pre_post_norm_counts.svg", pattern2 = "pre_post_norm_counts.svg")
    }
...
pca_heatmap.plot<-function(analysis_type = analysis_type, comparison = comparison){
  ...
  # Check for PCA folder (new location for all-sample normalization visualizations)
  if (dir.exists(paste0("../",analysis_type$DE[1],"/PCA"))) {
    output<-c(output,cat("#### Result over all condition {.tabset} \n"))
    for(j in 1:length(analysis_type$name)){
      output<-c(output,cat("##### ",analysis_type$name[j],"\n"))
      allpca.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = "PCA",path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "sample_to_sample_PCA.svg", pattern2 = "sample_to_sample_PCA_batch.svg")
      allheatmap.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = "PCA",path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "heatmaps_samples_log2.png", pattern2 = "heatmaps_samples_log_batch.png")
  ...
  for(i in 1:length(comparison)){
    ...
    pca.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = comparison, path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "sample_to_sample_PCA.svg", pattern2 = "sample_to_sample_PCA_batch.svg")
    heatmap.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = comparison,path.prefix = "../", path.suffix = "/DESeq2/report_data/", pattern1 = "heatmaps_samples_log2.png", pattern2 = "heatmaps_samples_log_batch.png")
    for(j in 1:length(analysis_type$name)){
      output<-c(output,cat("###### ",analysis_type$name[j]," \n"))
...
summary.table<-function(comparison=comparison, analysis_type = analysis_type){
  ...
  for(i in 1:length(comparison)){
    ...
    for(j in 1:length(analysis_type$DE)){
      de.tab.edger <- fread(paste0("../",analysis_type$DE[j],"/",comparison[i],"/edgeR/edgeR_", comparison[i], ".tsv"),
                          sep  = "\t",header = T)
      de.tab.deseq <- fread(paste0("../",analysis_type$DE[j],"/",comparison[i],"/DESeq2/detail_results/full_DESeq2.tsv"),
                          sep  = "\t",header = T)
...
volcano.ma.plots<-function(analysis_type = analysis_type, comparison=comparison){
  for(i in 1:length(comparison)){
    ...
    volcano.dt <- plot.dt(analysis.type = analysis_type$analysis, comp.folder = comparison,
                          path.prefix = "../", path.suffix = "/DESeq2/report_data/",
...
- Main output files are **DESeq2_<comparison>.tsv**, **DESeq2_noIndFilt_<comparison>.tsv**, and **edgeR_<comparison>.tsv**
...
- **DESeq2_<comparison>.tsv** – main results from differential gene expression by DESeq2. Description of the columns is given above. tsv is a tab-separated source file for the xlsx.
...
- **edgeR_<comparison>.tsv** – main results from differential gene expression by edgeR. tsv is a tab-separated source file for the xlsx.
